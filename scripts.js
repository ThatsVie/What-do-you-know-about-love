document.addEventListener("DOMContentLoaded", function () {
  ...
  function toggleReadMore(cardText, fullText, truncatedText, readMoreLink) {
    const isTruncated =
      cardText.innerHTML === highlightText(truncatedText, searchTerm);
    cardText.innerHTML = isTruncated
      ? highlightText(fullText, searchTerm)
    ...
    searchForm.addEventListener("submit", function (event) {
      ...
      allAnswers.forEach((answer) => {
        ...
        if (cardTitleElement) {
          ...
          const tagsText = tagsElement
            ? tagsElement.textContent.toLowerCase()
            : "";
          ...
          if (
            (searchTerm &&
              (cardTitle.includes(searchTerm) ||
                cardTextCombined.includes(searchTerm) ||
            ...
            if (searchTerm) {
              ...
              clonedTitle.innerHTML = highlightText(
                clonedTitle.innerHTML,
                searchTerm,
              );
              const clonedTextElements =
                answerClone.querySelectorAll(".card-text");
              ...
              if (clonedTags) {
                clonedTags.innerHTML = highlightText(
                  clonedTags.innerHTML,
                  searchTerm,
                );
              }
            }
  ...
  function insertBackToTopButtons(cards, container) {
    ...
    const cardsPerButton = isMobile ? 6 : 12;
